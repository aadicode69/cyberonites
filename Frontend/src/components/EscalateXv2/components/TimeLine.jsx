...
const TimeLine = () => {
    ...
    useEffect(() => {
        ...
        if (nextEvent) {
            setCurrentStatus(
                <>
                    <p className="font-bold text-lg text-gray-200 font-orbitron">Next Up: {nextEvent.event}</p>
                    <p className="text-gray-400 mt-2 font-mono text-sm">Scheduled for {nextEvent.date} at {nextEvent.time}. Stand by for engagement.</p>
                </>
            );
        } else {
            setCurrentStatus(
                <>
                    <p className="font-bold text-lg text-gray-200 font-orbitron">All events concluded.</p>
                    <p className="text-gray-400 mt-2 font-mono text-sm">Thank you for participating in EscalateX v2. See you next year!</p>
                </>
            );
        }
    }, [TIMELINE_DATA]);
                ...
                <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
                    <header className="mb-8">
                        <h2 className="text-4xl md:text-5xl font-black mb-2 font-orbitron">
                            ...
                            <span className="escalateX-neon-text escalateX-glitch" data-text="TIMELINE" style={{ color: "var(--escalate-orange)" }}>TIMELINE</span>
                        </h2>
                    ...
                    <div className="w-full max-w-md timeline-container">
                        {TIMELINE_DATA.map((item) => (
                            <div key={item.id} className={`timeline-item ${activeTimelineId === item.id ? 'is-active' : ''}`}>
                                <div className="timeline-marker"></div>
                                <p className="text-sm font-bold text-orange-400 font-orbitron tracking-wider">{item.phase} - {item.date}</p>
                                <h4 className="text-xl font-bold text-white my-1 escalateX-neon-text font-orbitron">{item.event}</h4>
                ...
                <div className="flex flex-col gap-8">
                    <div className="escalateX-glass-card p-8 escalateX-cyber-terminal escalateX-holographic">
                        <h3 className="text-2xl font-bold text-white mb-4 flex items-center gap-3 escalateX-neon-text font-orbitron">
                            <FaMapPin className="text-orange-400" />VENUE INFORMATION
                        </h3>
                        <p className="font-bold text-lg text-gray-200 font-orbitron">GLA University, Mathura</p>
                        <p className="text-gray-400 mt-2 font-mono text-sm">All offline activities (Day 1 & Day 2) will be conducted at GLA University campus in Mathura. Participants advancing from the Pre-CTF will receive detailed venue and accommodation information.</p>
                    </div>
                    <div className="escalateX-glass-card p-8 escalateX-cyber-terminal escalateX-holographic">
                        <h3 className="text-2xl font-bold text-white mb-4 flex items-center gap-3 escalateX-neon-text font-orbitron">
                            <FaInfoCircle className="text-orange-400" />CURRENT STATUS
                        </h3>
