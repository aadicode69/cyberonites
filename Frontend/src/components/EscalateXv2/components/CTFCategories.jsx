  ...
  const ctfCategories = [
    {
      ...
      icon: <FaGlobe />,
      description:
        "Attack and defend web applications through various vulnerability types.",
      difficulty: "HIGH",
    },
    {
      ...
      icon: <FaCog />,
      description:
        "Analyze and reverse binary files, executables, and compiled programs.",
      difficulty: "CRITICAL",
    },
    {
      ...
      icon: <FaMicroscope />,
      description:
        "Investigate digital evidence and recover hidden information from various sources.",
      difficulty: "HIGH",
    },
    {
      name: "OSINT",
      icon: <FaEye />,
      description:
        "Gather intelligence using open-source information and advanced investigative techniques.",
      difficulty: "MEDIUM",
    },
    {
      name: "Cryptography",
      icon: <FaLock />,
      description:
        "Break encryption, solve cryptographic puzzles, and find hidden information.",
      difficulty: "CRITICAL",
    },
    {
      name: "Miscellaneous",
      icon: <FaPuzzlePiece />,
      description:
        "Diverse challenges covering programming, logic puzzles, and creative problem-solving.",
      difficulty: "MEDIUM",
    },
    {
      ...
      icon: <FaDesktop />,
      description:
        "Complete system compromise from initial access to administrative privileges.",
      difficulty: "CRITICAL",
    },
  ];
  ...
  const positionCtfCards = useCallback(() => {
    ...
    cards.forEach((card, i) => {
      ...
      if (distanceAlongPerimeter <= rectWidth) {
        ...
        y =
          rectHeight / 2 -
          (distanceAlongPerimeter - 2 * rectWidth - rectHeight);
      }
  ...
  useEffect(() => {
    ...
    const handleScroll = (e) => {
      ...
      if (!selectedCard) {
        ...
        setCtfCircleRotation(
          (prevRotation) => prevRotation + scrollDelta * 0.2
        );
      }
    };
    ...
    const handleClickOutside = (e) => {
      if (
        !e.target.closest(".escalateX-ctf-card") &&
        !e.target.closest("#ctf-heading-container")
      ) {
      ...
      <div className="w-full max-w-7xl mx-auto h-full relative py-8 px-4 overflow-hidden">
        <div id="ctf-heading-container" className="escalateX-sci-fi-heading">
          ...
          <h2 className="text-4xl md:text-5xl font-black mb-2 font-orbitron relative z-10">
            ...
            <span
              className=" ml-2 cyber-glow-text"
          ...
          <p className="text-lg text-gray-300 max-w-xl font-mono opacity-90 mx-auto relative z-10 cyber-terminal-text">
            [SYSTEM] Day 2 features an intense 8-hour CTF championship. Use your
            scroll wheel to navigate the categories.
          </p>
        </div>
        <main id="ctf-grid" ref={ctfGridRef}>
          {ctfCategories.map((category, index) => (
            <article
              key={index}
              className={`escalateX-ctf-card escalateX-sci-fi-card ${
            ...
            >
              ...
              <header className="flex items-start gap-2 mb-2 relative z-10">
                ...
                <div className="flex-1 min-w-0 text-left">
                  <h3 className="text-white font-bold mb-1 font-orbitron text-left">
                    {category.name}
                  </h3>
                  <span
                    className={`inline-block px-2 py-1 text-xs border font-mono tracking-wider  ${getDifficultyClass(
                      category.difficulty
                    )}`}
                  >
                    {category.difficulty}
              ...
              <p className="text-gray-300 text-sm leading-relaxed font-mono overflow-hidden relative z-10 text-left">
