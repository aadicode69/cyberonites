...
const KnowledgeVillages = ({ className }) => {
  const villages = [
    ...
    },
    {
      ...
      icon: <FaSearch />,
      description:
        "Master open-source intelligence gathering techniques and tools for cyber investigations.",
      capacity: 30,
      topics: [
    ...
    },
    {
      ...
      icon: <FaGlobe />,
      description:
        "Learn advanced web application security testing and exploitation methodologies.",
      capacity: 30,
      topics: [
        "SQL Injection",
        "XSS Attacks",
    ...
    },
    {
      ...
      icon: <FaCloud />,
      description:
        "Understand cloud security fundamentals, misconfigurations, and secure deployment practices.",
      capacity: 30,
      topics: [
        "AWS Security",
        "Container Security",
    ...
    <section className={`py-16 px-6 relative ${className}`}>
      <header className="text-center max-w-4xl mx-auto mb-16">
        <h2 className="text-4xl md:text-5xl font-black text-white mb-6 tracking-wide exv2-font-cyberjunkies">
          ...
          <span className="text-orange-500">VILLAGES</span>
        </h2>
      ...
      {/* Villages Grid */}
      <main className="grid grid-cols-1 justify-center md:grid-cols-2 lg:grid-cols-3 gap-12 max-w-7xl mx-auto mb-16">
        {villages.map((village, index) => (
          <article
            key={index}
            className="exv2-village-card p-6"
          >
            {/* Village Header */}
            <header
              className="exv2-village-header display-flex flex-col align-center gap-4 text-center mb-6"
            >
              <aside
              ...
              <data
                ...
                className="inline-block px-3 py-1 bg-orange-500/20 text-orange-400 text-sm font-medium rounded-lg exv2-font-cybero"
              >
                Max Capacity :{" "}
                <span className="exv2-font-clouds">{village.capacity}</span>
              </data>
            </header>
            ...
            <section className="mb-4">
              <h4 className="text-white text-sm font-semibold mb-3 text-center exv2-font-clouds">
                Key Topics
              </h4>
              <ul className="flex flex-wrap gap-2 justify-center">
                {village.topics.map((topic, idx) => (
                  <li
                    key={idx}
                    className="px-3 py-1 bg-white/5 text-gray-300 text-xs rounded-lg border border-white/10"
                  >
                    {topic}
                  </li>
                ))}
      ...
      <footer className="text-center max-w-2xl mx-auto p-8 bg-gradient-to-br from-orange-500/5 to-orange-600/5 border border-orange-500/20 rounded-2xl backdrop-blur-lg">
        <h3 className="text-white text-xl font-bold mb-4 exv2-font-clouds">
          Village Access Protocol
        </h3>
        <p className="text-gray-300 text-base leading-relaxed mb-4">
          All Knowledge Villages run simultaneously on{" "}
          <strong>Day 1 (November 8th)</strong>. Participants can choose their
          preferred learning track during registration.
        </p>
